function Vmap(Divid){
	...
	function madeMap(unit_id,floorid,version){
		$.getJSON("http://"+ip+":"+port+"/vmapAPI/unit!around?client=824&vkey=FFE58998-B203-B44E-A95B-8CA2D6CBCD65&unit="+unit_id+"&floor="+floorid
        ...
        if(i!=-1){
            if (This.showBooth  ) {
                This.showBooth({
                    ...
                    block_state : Booths[i].block_state,
                    block_area : Booths[i].block_area,
                    block_tonnage : Booths[i].block_tonnage,
                    block_price : Booths[i].block_area * Booths[i].block_rent * Booths[i].block_discount,
                    block_show_name : Booths[i].company_show_name,
                    con_per : Booths[i].con_per,
                    phone : Booths[i].phone,
                    x : zoom_Booths[i].Pos_x,
                    y : zoom_Booths[i].Pos_y});
            }
            console.log(zoom_Booths[i]);
    ...
    function formatData(b,f){
        ...
        for(var i=0;i<b.length;i++){
            ...
            if(b[i].type_id<3000){
                ...
                booth.block_rent = b[i].block_rent;
                booth.block_tonnage = b[i].block_tonnage;
                booth.block_discount = b[i].block_discount;
                booth.company_show_name = b[i].company_show_name;
                booth.con_per = b[i].con_per;
                booth.phone = b[i].phone;
